...
export function buildAnimationClasses(
  ...
  if (animation) {
    classes.push(`doo-iconik-${animation}`);
  } else {
...
export function buildVariantClass(variant?: string): string {
  if (!variant || variant === 'default') return '';
  return `doo-iconik-${variant}`;
}

// CSS keyframes and classes for all animations + style variants
